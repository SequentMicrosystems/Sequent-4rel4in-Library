/*!
    Hv inputs usage example for Sequent Microsystems Four Relays four HV Inputs HAT
...
void setup() {
  ...
  if(card.cfgCount(1, true))
  {
	Serial.print("Counting enabled\n");
  }
  else
  {
	 Serial.print("Fail to enable transitions counting!\n");
  }
...
void loop() {
  if (card.isAlive() )
  {
	  Serial.print("Inputs state: ");
	  Serial.println(card.readOpto(), BIN);
	  if(card.readButton())// push the button to reset the conter on channel 1
	  {
		  card.resetCounter(1);
	  }
	  Serial.printf("Channel 1 Cnt: %d | fill: %0.2f%% | freq: %dHz\n", card.readCounter(1),  card.readFill(1), card.readFrequency(1));
  }
  delay(1000);
}
